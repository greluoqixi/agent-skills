#!/usr/bin/env python3
"""Sync existing CLAUDE.md files with current directory state.
...
Usage:
    python sync_claude_md.py <project-root> [--dry-run]
"""
...
def find_claude_md_files(root: Path) -> list:
    """Find all CLAUDE.md files in project, skipping ignored dirs."""
    ignored = {".git", "node_modules", "__pycache__", ".venv", "venv",
               "dist", "build", "target", ".next", ".turbo", ".claude"}
    results = []
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Sync CLAUDE.md files with manifest")
    ...
    existing_files = find_claude_md_files(root)
    report = build_sync_report(root)

    print("=== CLAUDE.md Sync Report ===")
    print(f"Project: {root}")
    print(f"Existing CLAUDE.md files: {len(existing_files)}")
    print()
    ...
    if actions["preserve"]:
        ...
        for item in actions["preserve"]:
            print(f"  \U0001f512 {item.get('file', '?')}/{item.get('section', '?')}")
        print()
