# AGENTS.md Template Specification (v1)
...
| Codex sandbox config | if config | if config | if config | if config | if config | if config | if config | if config | if config | if config |
...
2. **File existence** -- `if exists`, `if hooks`, `if sim dirs`, `if sim pkgs`, `if HIL tests` all check the filesystem at generation time. Boolean macros: `HAS_README`, `HAS_EXISTING_AGENTS`, `HAS_HOOKS`, `HAS_SIM_DIRS`, `HAS_SIM_PKGS`, `HAS_HIL_TESTS`, `HAS_CODE_SANDBOX` (for `.codex/config.toml`).
...
Sections evaluating to empty are omitted entirely from the final AGENTS.md.

## Content Quality Rules

### The Delete Test
For every line you write, ask: "If I delete this line, would Codex make a mistake?" If the answer is no, delete it.
...
| "Write clean code" | Nothing -- Codex already does this |
