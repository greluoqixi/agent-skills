#!/usr/bin/env python3
""".codex/.agents-md-state.json manifest manager.

Manages section state via a JSON manifest at .codex/.agents-md-state.json
using SHA-256 hashes. Used by both generation and sync workflows.
...
MANIFEST_DIR = ".codex"
MANIFEST_FILE = ".agents-md-state.json"
...
def save_manifest(root: Path, data: dict):
    """Write *data* to the manifest file, creating ``.codex/`` if needed."""
...
def compute_manifest_sections(filepath: Path) -> dict:
    ...
    This is the data that gets stored in the manifest for each AGENTS.md file.
    """
...
def update_manifest_after_generation(
    ...
    """Update manifest after generating (or regenerating) AGENTS.md files.
    ...
    files_generated:
        Mapping of logical manifest paths (e.g. ``"./AGENTS.md"``) to the
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Manage .codex/.agents-md-state.json manifest files"
    )
    ...
    update_parser = subparsers.add_parser(
        "update", help="Update manifest after generating AGENTS.md files"
    )
