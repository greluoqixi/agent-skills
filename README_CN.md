...
在上下文丢失前捕获来之不易的调试经验。每个生成的项目都附带 `TROUBLESHOOTING.md` 模板，采用两部分记录结构：

- **回溯** — 症状、根因链、修复步骤及文件路径
- **预防** — AI 为什么走错方向、下次做类似改动前应验证的检查规则、需要关注的上下游模块

记录使用时间衰减策略（6 个月新鲜度窗口，旧条目标注过期标签）。根 CLAUDE.md/AGENTS.md 通过 `@TROUBLESHOOTING.md` 引用知识库，维护规则中包含记录非显而易见修复的软约束。
